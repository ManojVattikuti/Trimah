...
const BC3 = () => {
  ...
  const ConsultingCard = ({ icon, title, description, variant = "default" }) => {
    ...
    return (
      <div
      className={`flex flex-col items-center px-8 py-5 w-full ${bgColor} shadow-[0px_1px_4px_rgba(0,0,0,0.15)] rounded-[30px] max-md:px-4 max-md:mt-8 transition-all duration-300 ease-in-out hover:shadow-xl hover:${bgColor}`}
    >
      <div
        className="flex z-10 flex-col justify-center items-center px-6 mt-0 bg-white rounded-full border-8 border-blue-50 border-solid h-[124px] w-[124px] max-md:px-4 transition-all duration-300 ease-in-out hover:bg-blue-100 hover:border-blue-300"
      >
        <img
          loading="lazy"
          src={icon}
          alt={`${title} icon`}
          className="object-contain aspect-square w-[50px] transition-transform duration-300 ease-in-out hover:scale-110"
        />
      </div>
      <div className={`mt-6 text-xl leading-none text-center ${textColor} transition-all duration-300 ease-in-out hover:text-blue-500`}>
        {title}
      </div>
      <div className={`mt-6 text-base leading-6 text-center ${descriptionColor} transition-all duration-300 ease-in-out hover:text-gray-500`}>
        {description}
      </div>
    </div>
  ...
  return (
    ...
    <div className="flex flex-col items-center py-4 px-32 w-full bg-blue-50 max-md:px-4 max-md:max-w-full relative"
    >
        <div
        ...
        <div className="flex flex-col w-full max-md:w-full">
          <div className="flex flex-col mt-16 max-md:mt-8 max-md:max-w-full">
            <div className="flex flex-col text-center">
              <h1 className=" text-3xl sm:text-3xl lg:text-4xl font-bold font-parkinsans">
                Our Consulting Expertise
              </h1>
              <p className="mt-2 text-sm sm:text-base lg:text-lg mb-4 font-parkinsans text-gray-500  max-md:max-w-full">
                We specialize in delivering expert consulting services across key IT domains
              </p>
            </div>
            <div className="mt-16 max-w-full w-full max-md:mt-8 font-parkinsans">
              {/* 3 cards on top */}
              <div className="flex gap-6 max-md:flex-col ">
    ...
    <div className="max-w-full w-[746px] mt-8 font-parkinsans ">
