...
const Messages = () => {
 ...
 const {messages,loading, error} = fetchMessages(token)
  ...
  const filteredMessages = filterDate
    ? messages.filter(message => moment(message.createdAt).format("YYYY-MM-DD") === filterDate)
    : messages;
  ...
  const downloadPDF = () => {
    ...
    // Define PDF columns
    const columns = ['Email', 'Name', 'Message', 'Created'];
    ...
    doc.autoTable({
      ...
      didDrawPage: () => {
        doc.text('Messages List', 90, 15); // Add title to the PDF
      }
    });

    // Save the PDF
    doc.save('messages_list.pdf');
  };
